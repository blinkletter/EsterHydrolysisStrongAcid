...
Converting from molality to molarity is trivial, but one needs to know the density, $\rho$, of the mixture.
\begin{equation}
...
\newpage
 ...
 The \textit{Python} code for fitting the model is available in the notebook described previously.\sidenote[][0mm]{See reference \vref{ref:modelpython}} I will produce the following plots\ldots
...
Figures~\ref{fig:plotB1} and \vref{fig:plotB2} are plots of density data using data sets that are available for \qty{25}{\degreeCelsius} and \qty{20}{\degreeCelsius}. I observe that the literature data is in good iagreement across the century of time between these data sets.

When I plotted the data from Rhodes,\tss{\ref{ref:rhodes}} I observed \textbf{five typographical errors} in the data set. The data at the concentration of \qty{51.2}{\percent\ce{H2SO4}} was off by \qty{+0.1}{\gram\per\centi\meter\cubed} in all four temperature series. That seems like too much of a coincidence. I corrected the data and made a note in the data tables presented on my \textit{GitHub} repository. Others can judge my decisions easily because I make my data and methods available as open source materials.

  

\begin{figure}
  \centering
  \caption{Plots of $\rho$ vs $w$ for data sets of Perry (small closed circles), Rhodes (green open circles) and Oca (red open circles) at \qty{25}{\degreeCelsius}\\ $\longleftarrow$ \\ \vspace{5mm} }
...
\begin{figure}
  \centering
  \caption[][10mm]{Plots of $\rho$ vs $w$ for data sets of Perry (small closed circles), Kaye (green open circles) and Oca (red circles) at \qty{20}{\degreeCelsius}\\ $\longleftarrow$} 
...
$$\rho = a_4 x^4 + a_3 x^3 + a_2 x^2 + a_1 x + a_0$$\\  \vspace{1mm} 
\noindent $x$ is mole fraction of \ce{H2SO4} and it returns the density, $\rho$, in \unit{\kilo\gram\per\meter\cubed}\\ \vspace{0mm}
\begin{align*}
a_0 &=  \num{997.05} & a_1 &=  \num{3420} \\
a_2 &=  \num{-7230} & a_3 &=  \num{9090} \\
a_4 &=  \num{-4410}
...
\begin{figure}
  \centering
  \caption{Plots a 4\tss{th}-degree polynomial model. \\ $\longleftarrow$ \\
...
\begin{figure}
  \centering
  \caption{Plots a 6\tss{th}-degree polynomial model. \\ $\longleftarrow$ \\
...
\begin{figure}
  \centering
  \caption{Plots a 20\tss{th}-degree polynomial model for $a_{H_2O}$. \\ $\longleftarrow$ \\
...
I tried lots of ways to improve the fit. Setting the weights to values of $\nicefrac{1}{a_{H_2O}}$ puts increasing weight toward the smaller values. This resulted in a fit that followed the data within \qty{2}{\percent}, but the $Python$ numerical engine warned of errors. We needed such a high degree of a polynomial we were getting past the limits of double-floating-point precision. Computers are machines and have physical limits.


\subsection{A Piecewise Model}

I tried splitting the data up into three sections to perform a piecewise fit. Each section still required high degrees of parameterization to get an adequate fit. Table~\ref{tab:piecewise} describes regions and models that were used.

\begin{table}[h!]
\caption{Pieces for the piecewise model. Small piece require fewer terms for an accurate fit. \\ $\longleftarrow$}
\centering
...
\begin{figure}
  \centering
  \caption{Plots a piecewise polynomial model for $a_{H_2O}$. \\ $\longleftarrow$ \\
...
The values for the coefficients are given in Table~\vref{tab:pieces}. Take note of the extreme degree of precision expressed. The high degree of the polynomials and the large range of magnitude in the values require every digit of precision that a 64-bit float can provide.
...
\begin{figure}
  \centering
  \caption{Plots a polynomial model for $\log{a_{H_2O}}$. \\ $\longleftarrow$ \\
...
\vspace{1mm}
\noindent \textsc{Lower Right}: This is the differential plot. The model fits poorly across the whole range. A difference of 0.1 in $\log{a_{H_2O}}$ is the same as a \qty{25}{\percent} error in the predicted value for $a_{H_2O}$.  
}
 \includegraphics[scale=0.75]{images/plot_D3} 
  \label{fig:plot_D3}
\end{figure}

Clearly, my amateur efforts have failed. For whatever reason, the shape of the $\log{a_{H_2O}}$ vs \unit{\percent\ce{H2SO4}} curve defies a good polynomial fit. The large change between the last and second-last values results in a poor fit. We do not have balanced data. The extremely steep slope at the end means that small imperfections in the model will yield large differences in the prediction. This high-concentration region was the most important in my analysis.

\clearpage

\subsection{Using Python Interpolation Functions}

I am using a polynomial fit to make a model for data that was likely derived from an undocumented polynomial model (I haven't found the description yet). Perhaps I should just interpolate between the points.
...
The model would simply be the data set and the \textit{Python} command to return values from the interpolation function. That actually seems simpler and more portable than the text-based description of my polynomial model in Table~\vref{tab:pieces}.
...
One approach is to combine the three data sets to train a polynomial model. I can use all the data and obtain the best fit. Or I could use a spline interpolation using selected data points that create a smooth set of precise points assembled in the order of increasing \unit{\percent\ce{H2SO4}}. 
\clearpage
\subsection{A Polynomial Model}

Again, I created a 12\tss{th}-degree polynomial model and fit it to the data. Figure~\vref{fig:plot_G2}. It diverged at the highest values of $m$. The differential plot shows how the fit split the difference between the Johnson and the Jorgenson data sets, and then began diverging as it moved into the Gillespie data at very high values of $w$. There are probably better ways to do this such as splitting it into pieces like before, but I feel that interpolation will be the best option here.
...
To use an interpolator it is required to have a set of single values in an increasing x-axis series. I could have averaged the values where there were duplicates at a given value of $m$. I decided to use only the Johnson and Gillespie data sets to cover the range from \num{2} to \qty{101}{\percent\ce{H2SO4}}. The data is precise and closely spaced, and so the interpolation is a good representation of the data. Figure~\vref{fig:plot_G3} shows the result.
...
What a long strange journey it has been. I began this exercise seeking a better way to interpolate the literature data sets for $\rho$, $a_{\ce{H2O}}$ \& $H_0$ of sulphuric acid mixtures. I was unhappy with the results of interpolation that I was using previously and wanted to try polynomial models for this interpolation. In the end, I am all the way back to recommending a simple interpolation of the points in the data table as the solution for most cases. In the case of $a_{\ce{H2O}}$ values, where there is an enormous difference in magnitude between the final data points at high concentrations, the polynomial fit might be best. However, it was a complicated piecewise model requiring extreme precision in the parameters. The simple interpolation was good enough.
...
\begin{figure}
  \centering
  \caption{Plots of $a_{H_2O}^M$ (\unit{\mole\per\litre}) vs $w$ (\unit{\percent\ce{H2SO4}}) for the data of Giauque and Rard combined (black) along with the data set presented by Cox (red, Table 3 in the paper; blue, Table 4). The green line is the calculated formal concentration of \ce{H2O} in the mixture.\\ $\longleftarrow$} 
%  \hspace*{-10mm}
